...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      if (isSignedIn && user && user.emailAddresses?.[0]?.emailAddress) {
        const { getUserByEmail, getUnreadNotifications } = await import("@/utils/db/actions");
  ...
  useEffect(() => {
    const fetchUserBalance = async () => {
      if (isSignedIn && user && user.emailAddresses?.[0]?.emailAddress) {
        const { getUserByEmail, getUserBalance } = await import("@/utils/db/actions");
        const dbUser = await getUserByEmail(user.emailAddresses[0].emailAddress);
        if (dbUser) {
  ...
  const handleNotificationClick = async (notificationId: number) => {
    const { markNotificationAsRead } = await import("@/utils/db/actions");
