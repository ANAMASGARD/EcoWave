...
export default function RewardsPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-semibold mb-6 text-gray-800 dark:text-gray-200">Rewards</h1>
      
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg flex flex-col justify-between h-full border-l-4 border-green-500 dark:border-green-400 mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Reward Balance</h2>
        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-center">
            <Coins className="w-10 h-10 mr-3 text-green-500 dark:text-green-400" />
            <div>
              <span className="text-4xl font-bold text-green-500 dark:text-green-400">{balance}</span>
      ...
      <div className="grid md:grid-cols-2 gap-8">
                ...
                <div key={transaction.id} className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                  <div className="flex items-center">
                    {transaction.type === 'earned_report' ? (
                      <ArrowUpRight className="w-5 h-5 text-green-500 dark:text-green-400 mr-3" />
                    ) : transaction.type === 'earned_collect' ? (
                      <ArrowUpRight className="w-5 h-5 text-blue-500 dark:text-blue-400 mr-3" />
                    ) : (
                      <ArrowDownRight className="w-5 h-5 text-red-500 dark:text-red-400 mr-3" />
                    )}
                    <div>
                  ...
                  <span className={`font-semibold ${transaction.type.startsWith('earned') ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
        ...
        <div>
          <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Available Rewards</h2>
          <div className="space-y-4">
            {rewards.length > 0 ? (
              rewards.map(reward => (
                <div key={reward.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{reward.name}</h3>
                    <span className="text-green-500 dark:text-green-400 font-semibold">{reward.cost} points</span>
                  </div>
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/30 border-l-4 border-yellow-400 dark:border-yellow-500 p-4 rounded-md">
                <div className="flex items-center">
                  <AlertCircle className="h-6 w-6 text-yellow-400 dark:text-yellow-300 mr-3" />
                  <p className="text-yellow-700 dark:text-yellow-200">No rewards available at the moment.</p>
                </div>
              </div>
            )}
          </div>
