...
export default function CollectPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-semibold mb-6 text-gray-800 dark:text-gray-200">Waste Collection Tasks</h1>
      ...
      <div className="mb-4 flex items-center">
        <Input
          ...
          className="mr-2 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
        />
        <Button variant="outline" size="icon" className="border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
              ...
              <div key={task.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                <div className="grid grid-cols-3 gap-2 text-sm text-gray-600 dark:text-gray-400 mb-3">
                  <div className="flex items-center relative">
                    <Trash2 className="w-4 h-4 mr-2 text-gray-500 dark:text-gray-400" />
                    <span 
                    ...
                    {hoveredWasteType === task.wasteType && (
                      <div className="absolute left-0 top-full mt-1 p-2 bg-gray-800 dark:bg-gray-900 text-white dark:text-gray-200 text-xs rounded shadow-lg z-10">
                        {task.wasteType}
                      </div>
                    )}
                  </div>
      ...
      {selectedTask && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Verify Collection</h3>
            <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">Upload a photo of the collected waste to verify and earn your reward.</p>
            <div className="mb-4">
              <label htmlFor="verification-image" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Upload Image
              </label>
            ...
            {verificationStatus === 'success' && verificationResult && (
              <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-md">
                <p className="text-green-800 dark:text-green-200">Waste Type Match: {verificationResult.wasteTypeMatch ? 'Yes' : 'No'}</p>
                <p className="text-green-800 dark:text-green-200">Quantity Match: {verificationResult.quantityMatch ? 'Yes' : 'No'}</p>
                <p className="text-green-800 dark:text-green-200">Confidence: {(verificationResult.confidence * 100).toFixed(2)}%</p>
              </div>
            )}
            {verificationStatus === 'failure' && (
              <p className="mt-2 text-red-600 dark:text-red-400 text-center text-sm">Verification failed. Please try again.</p>
            )}
            <Button onClick={() => setSelectedTask(null)} variant="outline" className="w-full mt-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
              Close
            </Button>
          </div>
        </div>
...
function StatusBadge({ status }: { status: CollectionTask['status'] }) {
  const statusConfig = {
    ...
    in_progress: { color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200', icon: Trash2 },
    completed: { color: 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200', icon: CheckCircle },
    verified: { color: 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200', icon: CheckCircle },
  }
